"""
Quick position-only IK tests for URDF modular robots (3–7 DOF).

Generates random joint-type configurations, builds URDF with functions.urdf,
parses it, and solves IK for random reachable-ish targets using the DLS solver.
Reports basic position error statistics.
"""
...
def random_config(rng, min_joints=3, max_joints=7):
    """
    Random joint-type config with light constraints:
    - DOF in [3, 7]
    - At least one rot360 overall
    - No more than 2 consecutive rot180
    - For n >= 5, at least 2 rot360
    """
    joint_types = ["rot180", "rot360"]
    for _ in range(50):  # retry budget
        ...
        while len(seq) < n:
            ...
            if (
                ...
                continue  # avoid >2 consecutive rot180
            seq.append(choice)
    ...
    # Fallback to unconstrained if retries exhausted
    n = rng.integers(min_joints, max_joints + 1)
...
def estimate_reach_radius(urdf_info):
    """
    Rough upper bound on reach: sum of translation magnitudes of fixed transforms.
    """
    reach = 0.0
...
def is_target_reachable_heuristic(urdf_info, target, margin=0.1):
    """
    Conservative reachability check using a sphere based on reach radius.
    """
    reach = estimate_reach_radius(urdf_info)
    # Allow a small margin beyond estimated reach
    return np.linalg.norm(target) <= reach * (1.0 + margin)


def sample_reachable_target_via_fk(urdf_info, rng, noise_std=0.01):
    """
    Sample a random joint vector within limits, take its FK as a reachable target,
    then add small Gaussian noise. Ensures reachability up to the added noise.
    """
...
def run_batch(trials=50, seed=0, top_k=5, save_path="ik_run_data.json"):
    ...
    for _ in range(trials):
        ...
        errors.append(err)
        if err < 0.01:  # 1 cm success criterion
            successes += 1
        records.append(
            {
                "dof": n,
